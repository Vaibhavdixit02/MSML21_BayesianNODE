...
d = load("ExtendedSpiralNeuralODE_Architecture1_500_500_0.50.jld")
############################### PLOTAND SAVE LOSS ##################################
scatter(d["losses"], ylabel = "Loss",  yscale= :log, label = "Architecture1: 1000 warmup, 500 sample")
...
pl = scatter(d["tsteps"], mean(d["ode_data"][1,:,:],dims=2), color = :red, label = "Data: Var1", title = "Spiral Neural ODE")
scatter!(d["tsteps"], mean(d["ode_data"][2,:,:], dims=2), color = :blue, label = "Data: Var2", xlabel = "t", ylims = (-5, 5))
for _ in 1:300
    resol = predict_neuralode(d["samples"][100:end][rand(1:400)][1:end-1])
    plot!(d["tsteps"], resol[1,:], alpha=0.04, color = :red, label = "")
    plot!(d["tsteps"], resol[2,:], alpha=0.04, color = :blue, label = "")
end
resol = predict_neuralode(d["samples"][100:end][rand(1:400)][1:end-1])
plot!(d["tsteps"], resol[1,:], alpha=0.04, color = :red, label = "")
plot!(d["tsteps"], resol[2,:], alpha=0.04, color = :blue, label = "")

idx = findmin(d["losses"])[2]
prediction = predict_neuralode(d["samples"][idx][1:end-1])

plot!(d["tsteps"], d["prediction"][1,:], color = :black, w = 2, label = "")
plot!(d["tsteps"], d["prediction"][2,:], color = :black, w = 2, label = "Best fit prediction")
...
datasize_f = 50
tspan_f = (0.0, 1.2)
tsteps_f = [d["tsteps"]; range(tspan[2], tspan_f[2], length = datasize_f - datasize)]
...
ode_data_f = cat(d["ode_data"], mean_ode_data_f[:, datasize+1:end] .+ 0.1 .* randn(size(mean_ode_data_f, 1), size(mean_ode_data_f, 2) - datasize, 30), dims = 2)
...
idx = findmin(d["losses"])[2]
prediction_f = predict_neuralode_f(d["samples"][idx])
...
for k1 in 301:500
    σ = d["samples"][k1][end]
    resol = predict_neuralode_f(d["samples"][k1][1:end-1])
    for k2 in 1:10
        _resol = resol .+ σ .* randn.()
        label = ""
...
err = zeros(200)
for k1 in 301:500
    prediction_f = predict_neuralode_f(d["samples"][k1])
    err[k1-300] = sum(abs.((prediction_f[1,datasize+1:end] .- mean_ode_data_f[1,datasize+1:end])./mean_ode_data_f[1,datasize+1:end])) + sum(abs.((prediction_f[2,datasize+1:end] .- mean_ode_data_f[2,datasize+1:end])./mean_ode_data_f[2,datasize+1:end]))
end
...
save("ExtendedSpiralNeuralODE_Architecture1_500_500_0.50.jld", "losses",losses, "ode_data", ode_data, "samples", samples, "prediction", prediction, "stats", stats, "idx", idx, "tsteps", d["tsteps"], "tsteps_f", tsteps_f )

samples = hcat(d["samples"]...)
samples_reshape = reshape(samples[rand(1:500), :]', (500, 1, 1))
Chain_Spiral = Chains(samples_reshape)
plot(Chain_Spiral)
autocorplot(Chain_Spiral; title = "Spiral ODE")